...
class Command(BaseCommand):
    help = "Calculates historically accurate platform metrics with temporal pinning and robust analytics"
    ...
    def calculate_for_date(self, target_date):
        ...
        secret_policies = (
            Secret.objects.filter(created_at__date__lte=target_date)
            .exclude(policy__isnull=True)
            .exclude(policy__exact={})
            .count()
        )
        agent_policies = (
            AgentRegistration.objects.filter(created_at__date__lte=target_date)
        ...
        # ──────────────────────────────────────────────
        # 2. GROWTH — DELTA ON SPECIFIC DATE
        # ──────────────────────────────────────────────
        ...
        # ──────────────────────────────────────────────
        # 3. ENGAGEMENT — HISTORICALLY ACCURATE ACTIVE USERS (DAU, WAU, MAU)
        # ──────────────────────────────────────────────
        # Fetch snapshots active up to target_date in a single lean query
        recent_snapshots = list(
            TelemetrySnapshot.objects.filter(
                Q(client_timestamp__date__range=[month_ago, target_date])
                | Q(
                    client_timestamp__isnull=True,
                    created_at__date__range=[month_ago, target_date],
                )
            ).values("user_id", "client_timestamp", "created_at")
        )

        snap_users_daily = set()
        snap_users_weekly = set()
        snap_users_monthly = set()
        ...
        for s in recent_snapshots:
            ...
            if not uid:
                continue
            dt = (
                s["client_timestamp"].date()
                if s["client_timestamp"]
                else s["created_at"].date()
            )
            if dt == target_date:
                snap_users_daily.add(uid)
            if week_ago <= dt <= target_date:
                snap_users_weekly.add(uid)
            if month_ago <= dt <= target_date:
                snap_users_monthly.add(uid)

        user_last_active_daily = set(
            User.objects.filter(last_active_at__date=target_date).values_list("id", flat=True)
        )
        active_users_daily = len(snap_users_daily | user_last_active_daily)

        user_last_active_weekly = set(
            User.objects.filter(
                last_active_at__date__range=[week_ago, target_date]
            ).values_list("id", flat=True)
        )
        active_users_weekly = len(snap_users_weekly | user_last_active_weekly)

        user_last_active_monthly = set(
            User.objects.filter(
                last_active_at__date__range=[month_ago, target_date]
            ).values_list("id", flat=True)
        )
        active_users_monthly = len(snap_users_monthly | user_last_active_monthly)
        ...
        projects_with_secrets = (
            Project.objects.filter(secrets__isnull=False, created_at__date__lte=target_date)
            .distinct()
            .count()
        )
        ...
        for s in target_snapshots_qs.values(
            "user_id", "command_executions", "integrations_active"
        ):
            uid = str(s["user_id"]) if s["user_id"] else None
            cmds = s["command_executions"]
            if cmds and isinstance(cmds, dict):
                for cmd, count in cmds.items():
                    raw_command_accumulator[cmd] += count

            ints = s["integrations_active"]
            if uid and ints and isinstance(ints, list):
        ...
        # ──────────────────────────────────────────────
        env_dist = {
            "development": Secret.objects.filter(
                environment="development", created_at__date__lte=target_date
            ).count(),
            "staging": Secret.objects.filter(
                environment="staging", created_at__date__lte=target_date
            ).count(),
            "production": Secret.objects.filter(
                environment="production", created_at__date__lte=target_date
            ).count(),
        }

        # ──────────────────────────────────────────────
        # 8. SAVE — ATOMIC UPDATE
        # ──────────────────────────────────────────────
        DailyMetricsAggregate.objects.update_or_create(
            ...
            defaults={
                ...
                "total_process_verifications_failed": proxy_stats[
                    "total_process_verifications_failed"
                ]
                or 0,
                "total_production_write_challenges": proxy_stats[
                    "total_production_write_challenges"
                ]
                or 0,
                ...
                "total_interactive_prompts_shown": proxy_stats["total_interactive_prompts_shown"]
                or 0,
                "total_interactive_prompts_skipped": proxy_stats[
                    "total_interactive_prompts_skipped"
                ]
                or 0,
                ...
                "total_identity_anonymous_calls": proxy_stats["total_identity_anonymous_calls"]
                or 0,
                ...
                "total_capability_violations_blocked": proxy_stats[
                    "total_capability_violations_blocked"
                ]
                or 0,
                "total_process_verifications_passed": proxy_stats[
                    "total_process_verifications_passed"
                ]
                or 0,
